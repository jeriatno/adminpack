    ...
    <select
        ...
        @if (isset($field['model']))
            @foreach ($options as $option)
                @if( (old(square_brackets_to_dots($field["name"])) && in_array($option->getKey(), old($field["name"]))) || (is_null(old(square_brackets_to_dots($field["name"]))) && isset($field['value']) && in_array($option->getKey(), collect($field['value'])->pluck($option->getKeyName(), $option->getKeyName())->toArray())))
