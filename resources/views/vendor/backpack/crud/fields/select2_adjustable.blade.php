...
<script>
    jQuery(document).ready(function($) {
        ...
        $("#select2_ajax_{{ $field['name'] }}").on('change', function(e) {
            let dataCust = $(this).select2('data')[0].data;

            if(dataCust.email != null) {
                $('#partner_email').val(dataCust.email);
            }

            if(dataCust.email != null) {
                $('#partner_address').val(dataCust.address);
            }

            if(dataCust.phone_number != null) {
                let phone = JSON.parse(dataCust.phone_number)
                $('#partner_phone_number').val(phone[0]?.phone);
            }

            if(dataCust.pic_name != null) {
                $('#pic_name').val(dataCust.pic_name);
                $('#pic_phone').val(dataCust.pic_phone);
                $('#pic_email').val(dataCust.pic_email);
            }
        });
