    ...
    <div class="array-container form-group">
                ...
                @foreach( $field['columns'] as $prop => $component)
                    @if($component['type'] == 'select')
                        <td>
                            ...
                            <select class="form-control input-sm" name="item{{$prop}}[]" type="text" ng-model="item.{{ $prop }}">
                                @foreach($component['data'] as $index => $value)
                    ...
                    @else
                        <td>
                            <input class="form-control input-sm" name="item{{$prop}}[]" type="text" ng-model="item.{{ $prop }}">
                        </td>
                    @endif

                @endforeach
                <td ng-if="max == -1 || max > 1" width="50px">
        ...
        <div class="array-controls btn-group m-t-10">
            <button ng-if="max == -1 || items.length < max" class="btn btn-sm btn-default" type="button"
                    ng-click="addItem()" onclick="refresh()"><i class="fa fa-plus"></i> {{trans('backpack::crud.add')}} {{ $item_name }}
            </button>
        </div>
