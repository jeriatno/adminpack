...
<li filter-name="{{ $filter->name }}"
    ...
    <div class="dropdown-menu">
        <div class="form-group backpack-filter m-b-0">
            <select id="filter_{{ $filter->name }}" name="filter_{{ $filter->name }}"
    ...
    <script>
        jQuery(document).ready(function ($) {
            ...
            $("select[name=filter_{{ $filter->name }}]").change(function () {
                ...
                @isset($filter->options['parent_component_child'])
                    console.log('wewe');
                $.ajax({
                    url: "{{$filter->options['parent_url']}}",
                    data: {
                        id: value,
                        role : '{{ $filter->options['parent_component_child'] }}'
                    },
                    success: function (res) {
                        @if(isset($filter->options['parent_component_child']))
                        $("select[name=filter_{{ $filter->options['parent_component_child'] }}]").html(null);
                        var option = new Option('', '');
                        $("select[name=filter_{{ $filter->options['parent_component_child'] }}]").append(option);
                        for (var i = 0; i < res.length; i++) {
                            var option = new Option(res[i].text, res[i].id);
                            option.selected = false;
                            $("select[name=filter_{{ $filter->options['parent_component_child'] }}]").append(option)
            ...
            $("li[filter-name={{ $filter->name }}]").on('filter:clear', function (e) {
                ...
                @if(isset($filter->options['parent_component']))
                $("li[filter-name={{ $filter->options['parent_component'] }}]").trigger("filter:clear");
                $("li[filter-name={{ $filter->options['parent_component'] }}]").removeClass('active');
                $("li[filter-name={{ $filter->options['parent_component'] }}] .select2").select2("val", "").trigger("change");
