...
FixHeader.prototype = {
    init: function () {
        ...
        this._cloneTableElement = '<table class="cloned_header table" style="display: none; position: fixed; top: 0; table-layout: fixed;"><colgroup></colgroup></table>';
    ...
    appendElements: function() {
        ...
        for ($i = 0; $i < self.colNum; $i++) {
			...
			if($i==0){
				self.cloneHeaderObj.find('colgroup').append("<col style='width: 13.2%'>");
			}else{
				self.cloneHeaderObj.find('colgroup').append("<col style='width: 6.2%'>");
            
			}
			//self.tableObj.find('colgroup').append("<col>");
        }
    },
