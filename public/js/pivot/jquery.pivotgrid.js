			...
			$.map(rows, function(row){
				...
				for(var i=0; i<fields.length; i++){
					...
					if(row[fields[i]]==null){
						...
						value += parseFloat(tmp3) || 0;
						//value += parseFloat(row[fields[i]]) || 0;
					}
					
				}

				row[opts.pivot.aggregate.column.field] = addCommas(value);
			});
		}
		return rows;
	}
	...
	function aggregateFooter(target, rows){
		...
		if (opts.pivot.aggregate && opts.pivot.aggregate.footer){
			...
			for(var i=0; i<fields.length; i++){
				...
				//console.log(value);
				footerRow[fields[i]] = addCommas(value);
			}
	...
	$.fn.pivotgrid.defaults = $.extend({}, $.fn.treegrid.defaults, {
		...
		operators:{
			sum: function(rows, field){
				...
				return addCommas(v.toFixed(opts.valuePrecision));
			},
